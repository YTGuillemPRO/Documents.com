...
🧱 Construcción: <span id="build">OFF</span><br>
🔫 Arma: <span id="weapon">Pistola</span> | Balas: <span id="ammo">∞</span>
`;
...
player.currentWeapon = "pistola";
player.ammo = {pistola:Infinity, rifle:30};
...
window.addEventListener("keydown", e=>{
  if(e.key==="1") player.currentWeapon="pistola";
  ...
  document.getElementById("ammo").textContent = player.ammo[player.currentWeapon];
});
...
window.addEventListener("mousedown", e=>{
  if(e.button===0){ // izquierdo
    if(player.ammo[player.currentWeapon]>0){
      ...
      bullet.dir = new THREE.Vector3(Math.sin(yaw),0,Math.cos(yaw)).multiplyScalar(-1);
      scene.add(bullet);
      bullets.push(bullet);
      if(player.currentWeapon!=="pistola") player.ammo[player.currentWeapon]--;
      document.getElementById("ammo").textContent = player.ammo[player.currentWeapon];
    }
  }
});
...
function buildFloor(){
  ...
  floor.position.set(Math.round(player.position.x/4)*4,0.15,Math.round(player.position.z/4)*4);
  scene.add(floor);
  builds.push(floor);
}

// ======================
// ZONA SEGURA / TORMENTA
// ======================
let zoneRadius = 150;
...
const zone = new THREE.Mesh(
  ...
  new THREE.MeshBasicMaterial({color:0x00e5ff,side:THREE.DoubleSide,transparent:true,opacity:0.3})
);
...
function animate(){
  ...
  if(player.position.y<=0){player.position.y=0;player.velY=0;player.onGround=true;}
  ...
  bullets.forEach((b,i)=>{
    ...
    npcs.forEach((npc,ni)=>{
      if(b.position.distanceTo(npc.position)<0.8){
        npc.hp -= player.currentWeapon==="pistola"?25:40;
        scene.remove(b);
        bullets.splice(i,1);
        if(npc.hp<=0){scene.remove(npc);npcs.splice(ni,1);player.btc+=10;}
      }
    });
  });
  ...
  npcs.forEach(npc=>{
    ...
    if(npc.position.distanceTo(player.position)<1){player.hp-=0.2;}
  });
  ...
  if(player.position.distanceTo(zoneCenter)>zoneRadius){player.hp-=0.3; zone.material.color.set(0xff0000); zone.material.opacity=0.5;} else {zone.material.color.set(0x00e5ff); zone.material.opacity=0.3;}
